...
export const getMatchStats = async (req: Request, res: Response) => {
  try {
    ...
    logger.info(`Fetching match stats for event ${id} in league ${league}`);

    const matchStats = await espnProvider.getMatchSummary(id, league);
...
export const getMatchPlayers = async (req: Request, res: Response) => {
  try {
    ...
    logger.info(`Fetching match players for event ${id} in league ${league}`);

    const matchStats = await espnProvider.getMatchSummary(id, league);
    const playEvents = await espnProvider.getPlayByPlay(id, league);
...
export const getPreGameContext = async (req: Request, res: Response) => {
  try {
    ...
    logger.info(`Fetching pre-game context for event ${id} in league ${league}`);

    const standings = await espnProvider.getStandings(league);
