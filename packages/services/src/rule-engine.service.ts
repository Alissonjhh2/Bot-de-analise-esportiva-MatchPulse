import { RuleEngineInput, RuleEngineOutput, ConditionEvaluationResult, RuleEngineConfig } from './rule-engine.types';
import { getStatValue, evaluateCondition, isWithinTimeRange } from './rule-engine.utils';
  ...
  evaluate(input: RuleEngineInput): RuleEngineOutput {
    ...
    if (strategy.status !== 'ACTIVE') {
      if (this.config.enableLogging) {
        console.log(`Strategy ${strategy.strategyId} is not active, skipping`);
      }
    ...
    if (!isWithinTimeRange(minute, strategy.startMinute, strategy.endMinute)) {
      if (this.config.enableLogging) {
        console.log(`Strategy ${strategy.strategyId} not in time range (${strategy.startMinute}-${strategy.endMinute}), current minute: ${minute}`);
      }
      return this.createOutput(matchId, strategy, minute, false, [], []);
    }
    ...
    if (this.config.enableLogging && allPassed) {
      console.log(`🎯 STRATEGY MATCHED! Strategy: ${strategy.name} (${strategy.strategyId}) at minute ${minute}`);
    }
